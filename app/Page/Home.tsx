import { View, Text, StyleSheet, SafeAreaView, TouchableOpacity, TextInput, FlatList, ActivityIndicator } from 'react-native';
...
export default function Home() {
  ...
  const [loading, setLoading] = useState<boolean>(false); // Loading state for API data fetching

  // Fetch and set location on mount
  useEffect(() => {
    (async () => {
  ...
  const searchPlaces = () => {
    ...
    fetch(
      `https://api.foursquare.com/v3/places/search?query=${search}&ll=${location?.coords.latitude}%2C${location?.coords.longitude}&radius=1000`,
      options
    )
      .then((res) => res.json())
      .then((res) => {
  ...
  const singlePlace = (item: any) => {
    ...
    setPlaces(null); // Hide places list after selection
  };
